...
export function _BaseForm({
  ...
  const DeleteToggle = <Switch onChange={setIsDelete} checked={isDelete} />

  const VipPointsToggle = (
    <Switch onChange={setIsVipPoints} checked={isVipPoints} />
  )
    ...
    return arr.map(
      ({
        ...
        <Col span={span} key={id}>
          <Form.Item label={label} labelAlign={labelAlign} {...layout}>
            {component
              ? component
  ...
  function renderRow(arr) {
    return arr.map(
      ({ type = 'flex', justify = 'space-between', children }, index) => (
        <Row key={index} type={type} justify={justify}>
          {renderCol(children)}
        </Row>
      )
    )
  ...
  const allowSubmit =
    ...
    data['category_name'] &&
    data['category_name'].trim().length >= 1
  ...
  async function handleUpdateCommodity() {
    ...
    try {
      const { data } = await CommodityTasks.updateCommodity(
        ajax,
        selectCommodity.barcode,
        new_data
      )
      closeDrawer()
  ...
  return (
    <Form className={styled['commodity-drawer-form']} onSubmit={handleSubmit}>
      {renderRow(common)}
      <Form.Item
        style={{
          marginTop: 20
