  ...
  return (
    <div className={styled['supplier-search']}>
      ...
      {btnList
        ...
        .map(({ title, key, fn, type, icon = '', defaultCheck = false }) => (
          <Button
            icon={icon}
            type={type}
            key={key}
...
export function Supplier() {
  ...
  const isDefaultSupplierFlag =
    supplier === undefined ? false : supplier.name === DEFAULT_SUPPLIER_NAME
...
function SupplierList({ list, select, selectType, handleClick }) {
  const footerData = useMemo(
    () => SupplierListFooterColumns(list.length),
    [list.length]
  )
...
function _SupplierModal({
  ...
  return (
    ...
    >
      <Form {...formItemLayout}>
        {formConfig.map(({ label, field, placeholder, config }) => (
