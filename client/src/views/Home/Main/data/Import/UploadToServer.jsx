...
export function UploadToServer({
    ...
    },
    {
      ...
      component: (
        <Result
          ...
          extra={[
            <Button type="primary" key="done" onClick={handleUpload}>
              重试
            </Button>
          ]}
        />
    ...
    },
    {
      ...
      component: (
        <Result
          ...
          }条数据, 创建${resultData.create_count}条数据, 更新${
            resultData.update_count
          }条数据, 跳过${resultData.skip_count}条数据。`}
          extra={[
            <Button type="primary" key="done" onClick={doneFn}>
              继续导入
            </Button>
          ]}
        ></Result>
  ...
  const { component } = useMemo(
    () => config.find((i) => i.status === uploadStatus),
    [uploadStatus]
  )

  return <div className={styled['upload-toserver-wrap']}>{component}</div>
}
