...
function renderNodes(tree, isSelect = false) {
  ...
  return tree.map(({ id, name, children }) => {
    if (children) {
      ...
      return <TreeNode icon={<Icon type="tag" />} title={name} key={id} />
    }
  })
}

export function Categories() {
  const { tree, checkedKeys, expandedKeys, selectKeys, categoryList } =
    useSelector((state) => state.warehouse.categories)
  ...
  const RightMenu = useMemo(() => {
    ...
    const items = isBaseTree
      ? baseItems.filter((i) => i.base_tree)
      : baseItems.filter((i) => (isParent ? !i.not_parent : !i.not_child))
    // 根据触发右键菜单的节点是否是一级分类筛选出需要的菜单

    const isDisabled = !!(
      isParent && categoryList.find((i) => i.parent_id === key)
    )
    // 删除菜单是否禁用的flag
    ...
    const menu = (
      ...
      >
        {items.map(({ title, fn, check_css }) => {
          ...
          check_css && isDisabled
            ? css.push(styled['disable'])
            : (event['onClick'] = fn)

          return (
            <div className={css.join(' ')} key={title} {...event}>
              {title}
            </div>
          )
        })}
...
function RightMenuModal({
  ...
  async function changeCateParent() {
    try {
      await CategoriesTask.updateCategoryParent(
        ajax,
        child_name,
        new_parent_name
      )
    ...
    status &&
      (() => {
        switch (_type) {
          ...
          case 'edit_parent':
            ;(() => {
              let { name: child_name, parent_id } = categoryList.find(
                (i) => i.id === selectKey
              )
              setParent(() => ({
                child_name,
                old_parent_name: parent_id
                  ? categoryList.find((i) => i.id === parent_id).name
                  : '',
                new_parent_name: ''
              }))
            })()
      ...
      <Form {...formItemLayout}>
        ...
        {isChangeName && (
          <Form.Item label="旧分类名称">
            <Input value={old_name} disabled />
          </Form.Item>
        )}
        {isChangeName && (
          <Form.Item label="新分类名称">
            <Input value={new_name} onChange={handleNewName} />
          </Form.Item>
        )}
        {isEditParent && (
          <Form.Item label="子分类名称">
            <Input value={child_name} disabled />
          </Form.Item>
        )}
        {isEditParent && (
          <Form.Item label="旧父分类">
            <Input value={old_parent_name} disabled />
          </Form.Item>
        )}
        {isEditParent && (
