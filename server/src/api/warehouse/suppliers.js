...
route.post(
  '/create',
  validBody(createSupplierSchema, '请输入正确的供应商信息!'),
  async (req, res, next) => {
    // 建立新的供应商信息
...
route.put(
  '/update',
  validBody(updateSupplierSchema, '请输入正确的供货商信息!'),
  async (req, res, next) => {
    // 修改供货商信息
    const { name, update_value } = req.body

    if (name === default_supplier)
      return throwError(next, '不能修改默认供货商!')
    ...
    if (new_name) {
      const queryNewSupplierNameResult = await SuppliersTask.getSupplierDetails(
        new_name
      )
