...
route.post(
  '/',
  validBody(addCommoditySchema, '请输入正确的促销活动详情!'),
  async (req, res, next) => {
    // 向促销活动中增加商品
    ...
    const { barcode, promotion_type, discount_value } = commodity

    const { status, message, data, promoCommodityExist } =
...
route.put('/', validBody(editCommoditySchema), async (req, res, next) => {
  ...
  const { status, message, data, promoCommodityExist } =
    await PromotionTask.checkCommodity(
      queryPromotionResult.id,
      barcode,
      promotion_type
    )
