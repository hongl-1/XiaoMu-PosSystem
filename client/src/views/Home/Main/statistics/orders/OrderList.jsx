...
export function OrderList({
  ...
  const [data, footerData] = useMemo(() => {
    ...
    function getRate(profit, sale_price) {
      ...
      const value = mathc.formatRate(mathc.divide(profit, sale_price))
    ...
    order_list.forEach(({ check_date, is_undo, ...fields }) => {
      data.push({
        ...
        profit_rate: getRate(fields['profit'], fields['sale_price'])
      })
  ...
  return (
    <div className={styled['order-list']}>
      <LoadingBox status={load} />
      <VirtualSelectList
