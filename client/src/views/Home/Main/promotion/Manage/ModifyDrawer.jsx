...
function _ModifyDrawer({
  ...
  const required_id = useMemo(
    () =>
      children.reduce((list, { id, rules }) => {
  ...
  function getValues() {
    ...
    const [start_date, end_date] = date_range.map((time) =>
  ...
  return (
    <Drawer visible={status} onClose={closeFn} title={title} width={515}>
      <Form {...formLayout}>
        {renderItem(children)}
        <Form.Item label=" " colon={false}>
          <Button type="primary" disabled={hasError} onClick={handleFn}>
            {okText}
          </Button>
        </Form.Item>
      </Form>
