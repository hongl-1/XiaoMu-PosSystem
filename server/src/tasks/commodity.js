...
class CommodityTask {
  static async mapCommdityCateSupp(list) {
    ...
    return await Promise.all(
      list.map(async ({ category_id, supplier_id, ...args }) => ({
        ...args,
  ...
  static async getCommodityByCategory(category) {
    ...
    if (Array.isArray(category)) {
      const result = await Promise.all(
        category.map((t) => this.getCommodityByCategory(t))
      )
    ...
    if (parent_id) {
      ...
      const result = (
        await Promise.all(childList.map(async ({ id }) => await query(id)))
  ...
  static async createCommodity({
    ...
    const { id: supplier_id } = await SuppliersTask.getSupplierDetails(
      supplier_name
    )
  ...
  static async updateCommodityCategoryID(barcode, new_category_name) {
    ...
    const { id: category_id } = await CategoriesTask.getCategoryDetails(
      new_category_name
    )
    return await AppDAO.run(
      `
  ...
  static async updateCommodityValue(update_value) {
    ...
    await this.updateCommodityChangedate(snapshot['id'], update_time)
    // 修改商品的修改时间
