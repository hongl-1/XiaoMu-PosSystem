  ...
  switch (action.type) {
    ...
    case WARE_CATEGORY_DELETE:
      return (() => {
        ...
        const checkedKeys = state.checkedKeys.includes(keys)
          ? state.checkedKeys.filter((i) => i !== keys)
          : state.checkedKeys

        const selectKeys = state.selectKeys.includes(keys)
          ? []
          : state.selectKeys

        const currentShowKeys = state.currentShowKeys.includes(keys)
          ? state.currentShowKeys.filter((i) => i !== keys)
          : state.currentShowKeys
