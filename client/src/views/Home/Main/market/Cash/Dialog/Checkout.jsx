...
function DeviceStatus({ value, status, fn, hotkey }) {
  return (
    <div>
      <Switch checked={status} onChange={fn} />
      &nbsp;
      {value}
      {`<${hotkey}>`}
    </div>
...
export function Checkout({
  ...
  const [printStatus, setPrintStatus] = useState(
    localStorage.getItem(GLOBAL_FRONT_AUTO_PRINT_STATUS) !== 'hide'
  )
  // 是否自动打印小票

  const [moneyBoxStatus, setBoxStatus] = useState(
    localStorage.getItem(GLOBAL_FRONT_BOX_STATUS) !== 'hide'
  )
  // 结账完成自动打开钱箱
  ...
  const checkPassFlag =
    list.length > 0 &&
    String(show_client_pay) === String(client_pay) &&
    change >= 0
  // 当前状态是否可以结账的flag
  ...
  function check() {
    ...
    async function end() {
      ...
      const commodity_list = list.map(
        ({ barcode, origin_price, sale_price, count, status }) => {
  ...
  useEffect(() => {
    status &&
      setTimeout(() => {
        ...
        current.focus()
        current.input.setSelectionRange(0, 10)
      })
  }, [status])
  ...
  return (
    ...
    >
      <div className={styled['wrap']}>
        <div>
          <Statistic title="应付金额" value={origin_price} precision={2} />
          <Statistic
            title="实际金额"
            value={sale_price}
            ...
            precision={2}
          />
          <Statistic title="实付金额" value={show_client_pay} precision={2} />
          <Statistic
            title="找零金额"
            value={change}
        ...
        >
          <div className={styled['device-status-wrap']}>
            <p>设备状态: </p>
