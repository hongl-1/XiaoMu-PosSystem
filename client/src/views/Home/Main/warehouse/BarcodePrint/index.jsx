...
export function BarcodePrint() {
  ...
  async function handleSearch(_query, selectInput) {
    ...
    if (query === '') {
      queryCommodityData.list.length !== 0 &&
        setQueryCommodityData(initQueryState)
  ...
  return (
    <div className={styled['barcodeprint-wrap']}>
      <SearchFn handleSearch={handleSearch}>
        {
          useMemo(() =>
            btnList.map(({ label, type = 'primary', fn, disabled, icon }) => (
      ...
      <SearchResult handleClick={handleQueryClick} {...queryCommodityData} />
      <CommodityList handleClick={handlePrintClick} {...printCommodityData} />
    </div>
  )
}
