...
function CustomSwitch({ status, label, toggleFn }) {
  return (
    <div>
      {label}:&nbsp;&nbsp;&nbsp;&nbsp;
...
const selector = ({ showTabs, showCashHotKey }) => ({
  showTabs,
  showCashHotKey
})

export function SettingDrawer({ show, hide }) {
  const { showTabs, showCashHotKey } = useSelector(selector)

  const dispatch = useDispatch()

  const { toggleTabs, toggleCashHotKey } = useMemo(
    () => ({
  ...
  function toggleMulitTabs(bool) {
    bool
      ? localStorage.removeItem(GLOBAL_TABS_STATUS)
      : localStorage.setItem(GLOBAL_TABS_STATUS, 'hide')
    toggleTabs(bool)
  }

  function toggleCashHotKeyStatus(bool) {
    bool
      ? localStorage.removeItem(GLOBAL_CASH_HOTKEY_SHOW)
      : localStorage.setItem(GLOBAL_CASH_HOTKEY_SHOW, 'hide')
    toggleCashHotKey(bool)
  }

  return (
    <Drawer visible={show} onClose={hide}>
      <CustomSwitch
        label="启用多标签"
        status={showTabs}
        toggleFn={toggleMulitTabs}
      />
      <CustomSwitch
        label="启用收银热键列表"
        status={showCashHotKey}
        toggleFn={toggleCashHotKeyStatus}
      />
    </Drawer>
  )
}
