    ...
    checkFn: ([start_time_stamp, end_time_stamp]) => {
      ...
      if (Number(end_time_stamp) - Number(start_time_stamp) > maxTime) {
        notification.warn({
          message: '超出查询范围',
          description:
            '按小时查询时最多可查询24小时的数据，超出将仅查询结束时间当天24小时的数据。'
        })
      }
    }
  },
    ...
    checkFn: ([start_time_stamp, end_time_stamp]) => {
      ...
      if (Number(end_time_stamp) - Number(start_time_stamp) > maxTime) {
        notification.warn({
          message: '超出查询范围',
          description:
            '按小时查询时最多可查询366天的数据，超出将仅查询结束时间之前366天的数据。'
        })
      }
    }
  },
    ...
    checkFn: ([start_time_stamp, end_time_stamp]) => {
      ...
      if (Number(end_time_stamp) - Number(start_time_stamp) > maxTime) {
        notification.warn({
          message: '超出查询范围',
          description:
            '按小时查询时最多可查询12个月的数据，超出将仅查询结束时间之前12个月的数据。'
        })
      }
    }
  }
...
export function Trends() {
  ...
  async function getTrendData() {
    ...
    try {
      const { data } = await StatisticsTasks.querySalesTrends(
        ajax,
        timerange,
        queryType
      )
