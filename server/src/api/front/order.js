...
route.post(
  '/submit',
  validBody(createOrderSchema, '订单信息不正确!'),
  async (req, res, next) => {
    // 创建新的订单
    ...
    const priceIsEqual = await OrdersTask.validOrderPrice(
      sale_price,
      commodity_list
    )
    ...
    const changeIsValid = await OrdersTask.validChange(
      client_pay,
      change,
      sale_price
    )
...
route.put(
  '/undo',
  validBody(undoOrderSchema, '订单号格式不正确!'),
  async (req, res, next) => {
    // 撤销订单，将订单作废处理
...
route.put(
  '/addvip',
  validBody(addOrderVipSchema, '请提交正确的订单号或会员号!'),
  async (req, res, next) => {
    // 追加VIP积分
...
route.get('/', async (req, res, next) => {
  ...
  const result = await Promise.all(
    data.map(({ order_id }) => OrdersTask.getOrderAllDetails(order_id))
  )
