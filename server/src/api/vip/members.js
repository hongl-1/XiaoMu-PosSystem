...
route.put(
  '/pointrules',
  validBody(setVipPointRuleSchema, '请输入正确的积分值!'),
  async (req, res) => {
    // 设置会员积分比例
...
route.post(
  '/create',
  validBody(createVipMemberSchema, '请输入正确的会员信息!'),
  async (req, res, next) => {
    // 创建新会员
...
route.put(
  '/update',
  validBody(updateVipMemberSchema, '请输入正确的会员信息!'),
  async (req, res, next) => {
    // 更新会员信息
...
route.post(
  '/change',
  validBody(changeVipMemberSchema, '请输入正确的会员卡号!'),
  async (req, res, next) => {
    // 会员补换卡
    ...
    const checkOldIsChangeResult = await VipTasks.checkVipMemberIsChange(
      old_code
    )
    if (checkOldIsChangeResult) {
      return throwError(next, '此卡已补换过卡!')
...
route.put(
  '/setpoint',
  validBody(setVipPointSchema, '请输入正确的调整信息!'),
  async (req, res, next) => {
