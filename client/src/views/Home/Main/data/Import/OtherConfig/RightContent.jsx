...
export function RightContent({
  ...
  const config = [
    {
      label: '撤消',
      component: (
        <Button type="danger" onClick={cleanMapValue}>
          <Icon type="undo" />
          撤销所有映射
        </Button>
      )
    },
    {
      label: '属性名',
      component: (
        <Select onChange={handleCurrentField} value={currentField.field}>
    ...
    },
    {
      label: '属性值',
      component: (
        <Select onChange={handleCurrentValue} value={currentField.value}>
  ...
  return (
    <Form {...formLayout} className={styled['right-wrap']}>
      {config.map(({ label, component, ...args }) => (
        <Form.Item label={label} key={label} {...args}>
          {component && component}
        </Form.Item>
      ))}
    </Form>
