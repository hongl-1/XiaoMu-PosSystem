...
export function PromotionCommodityManage() {
  ...
  const [commodityList, setCommodityList, flushList] = useCommodity(
    currentPromo,
    ajax,
    () => setLoadStatus(false)
  )
  ...
  const promoList = useMemo(
    () => basePromoList.map(({ name }) => name),
    [basePromoList]
  )
  // map出促销活动名称

  const selectCommodity = useMemo(
    () => commodityList.find((i) => i.id === selectData.selectId),
    [selectData.selectId, currentPromo]
  )
