...
route.get('/', async (req, res) => {
  ...
  const data = await Promise.all(
    list.map(async ({ id, supplier_id, date, description }) => {
      const { name: supplier_name } = await SuppliersTask.getSupplierDetails(
        supplier_id
      )
      return {
        id,
...
route.post(
  '/create',
  validBody(createStockSchema, '请提交正确的进货单信息!'),
  async (req, res, next) => {
    // 创建进货单

    const { supplier_name, description, commodity_list } = req.body
    const querySupplierResult = await SuppliersTask.getSupplierDetails(
      supplier_name
    )
    ...
    const id = await StockTask.createStock(
      querySupplierResult.id,
      data,
      description
    )
