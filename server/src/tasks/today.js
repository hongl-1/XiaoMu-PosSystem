...
import {
  getNightTimeStrap,
  getWeekendStartTimeStrap,
  getFormatTime,
  getNextDayStartTimeStrap
} from '../lib/time.js'
...
export class TodayTasks {
  static async getTodayOrderCount(start_time, end_time) {
    ...
    const order_list = await OrderTasks.getOrdersByTimerange(
      start_time,
      end_time,
      true
    )
  ...
  static async getAdminTodayData(username) {
    ...
    const category_data = (
      await StatisticsTasks.getCommoditySalesProportionByTime(
        start_time,
        end_time,
        'category$all'
      )
    ...
    const weekend_data = (
      await StatisticsTasks.getSalesTrends({
    ...
    const { vip_order_count, order_count } = await this.getTodayOrderCount(
      start_time,
      end_time
    )
  ...
  static async getCashierTodayData(username) {
    ...
    const weekend_order = await OrderTasks.getOrdersByTimerangeAndUser(
      weekend_start_time,
      end_time,
      user_id,
      true
    ...
    for (let { sale_price, count, check_date } of weekend_order) {
    ...
    for (let key of Object.keys(weekend_data_values)) {
      ...
      weekend_data.push([key, sale_price, count])
    }

    // sale_price, order_count, count, vip_order_count,

    const todayOrderList = weekend_order.filter(
      (o) => o.check_date >= start_time
    )
