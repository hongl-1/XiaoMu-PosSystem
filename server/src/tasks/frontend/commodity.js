...
class CommodityTask {
  static async getCommodityDetails(query, warequeryFlag = false) {
    ...
    const need_fields = (
      warequeryFlag ? ware_field_list : front_field_list
    ).join(', ')
  ...
  static async parseCommodityList(list) {
    ...
    if (!promo_list) {
      ...
      const promo_commodity_list = await PromotionTask.getCommodityByValidPromo(
        promo_id_list,
        commodity_id_list
      )
      // 参加活动的商品信息
      ...
      return list.map(
        ({ id, barcode, sale_price, vip_points, is_delete, ...keys }) => {
          const values = promo_commodity_list.find(
          ...
          if (!values) {
            ...
            const { promotion_type_id, discount_value } = values

            const { name } = promo_type_list.find(
