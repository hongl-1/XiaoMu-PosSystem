...
function formatValue(value) {
  ...
  if (index === -1) {
    return <span className={styled['value']}>{str}</span>
  }
  ...
  return (
    <span className={styled['value']}>
      {int}.<span className={styled['float']}>{float}</span>
    </span>
  )
}
...
export function BarcodeInputAndTotal({
  ...
  async function queryCommodity() {
    ...
    try {
      const { data } = await CommodityTasks.query(
        ajax,
