...
function Add({ toggleToAdd }) {
  ...
  return config.map(({ label, fn, type = 'primary' }) => (
    <Button key={label} type={type} onClick={fn}>
      {label}
    </Button>
  ))
}
...
function _AddForm({ toggleToDetails, form, addStock }) {
  ...
  const Supplier = (
    <SupplierSelect ajax={ajax} value={supplier} handleChange={setSupplier} />
  )
  ...
  async function submitStockOrder() {
    ...
    const value = {
      ...
      commodity_list: list.map(({ barcode, count, in_price }) => ({
        barcode,
        count,
        in_price
      }))
      ...
      <Form {...formLayout}>
        {config.map(
          ({
            ...
            type === 'btn' ? (
              <Button onClick={fn} type={btnType} key={id} disabled={disabled}>
                {label}
              </Button>
            ) : (
...
export function StockFn({ type, toggleType, addStock }) {
  ...
  const Component =
    type === 'details' ? (
      ...
      <AddForm toggleToDetails={toggleToDetails} addStock={addStock} />
    )

  return <div className={styled['stock-fn']}>{Component}</div>
}
