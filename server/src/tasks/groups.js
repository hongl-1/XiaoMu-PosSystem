...
class GroupTask {
  static async getAllGroup() {
    ...
    const result = await Promise.all(
      groupList.map(async ({ usergroup, id }) => {
        return {
          usergroup,
          id,
  ...
  static async createGroup(group, authorityIDList) {
    // 创建一个新的用户组
    ...
    await Promise.all(
      authorityIDList.map((id) => this.setGroupAuthority(lastID, id))
    )
  }
  ...
  static async setGroupAuthority(group_id, authority) {
    ...
    if (Array.isArray(authority)) {
      return Promise.all(authority.map(async (id) => await addID(id)))
    }
