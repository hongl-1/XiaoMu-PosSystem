...
function _FieldsConfig({ data, option, typeConfig, form }) {
  const { getFieldDecorator } = form

  const { type, fields, rules } = typeConfig.find(
  ...
  function renderRules(rules) {
    return rules.map(({ id, label, defaultValue, option }) => (
      <Form.Item key={id} label={label} {...rulesLayout} labelAlign="left">
  ...
  return (
    <div className={styled['fields-config-wrap']}>
      <p>
        {`上传的文件共有${data.length}条数据，拥有${
          (data[0] && Object.keys(data[0]).length) || 0
        }个字段`}
      </p>

      <Form>
        <Form.Item {...rulesLayout} labelAlign="left" label="导入数据类型">
