...
function getFormatType(list) {
  ...
  if (child) {
    ...
    result.push(childItem ? childItem.label : child)
  }
...
export function Proportion() {
  ...
  async function getData() {
    ...
    try {
      const { data } =
        await StatisticsTasks.queryCommoditySalesProportionByTime(
          ajax,
          timerange,
          currentType
        )
  ...
  useEffect(() => {
    ...
    if (parent_categories.length === 0 && category_config.children.length === 0)
      return
