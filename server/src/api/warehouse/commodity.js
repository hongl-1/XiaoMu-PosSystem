...
route.post(
  '/create',
  validBody(createCommoditySchema, '商品信息不正确!'),
  async (req, res, next) => {
    // 建立新的商品
    ...
    if (barcode) {
      const validBarcodeResult = await CommodityTask.getCommodityDetails(
        barcode
      )
    ...
    const validCategoryResult = await CategoriesTask.getCategoryDetails(
      category_name
    )
    ...
    if (supplier_name) {
      const validSupplierResult = await SuppliersTask.getSupplierDetails(
        supplier_name
      )
    ...
    const queryCommodityResult = await CommodityTask.getCommodityDetails(
      lastID,
      'id'
    )
...
route.put(
  '/update',
  validBody(updateCommoditySchema, '提交的商品信息不正确或不完整!'),
  async (req, res, next) => {
    // 更新商品信息
    ...
    if (barcode) {
      const validNewBarcodeResult = await CommodityTask.getCommodityDetails(
        barcode
      )
