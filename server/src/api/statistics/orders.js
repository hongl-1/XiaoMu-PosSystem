...
route.get(
  '/',
  validBody(ordersSchema, '请检查时间范围!', false),
  async (req, res, next) => {
    // 根据时间范围查询销售数据
...
route.get(
  '/query',
  validBody(queryOrdersSchema, '提供的数据不正确!', false),
  async (req, res, next) => {
    // 根据提供的数据查询相关订单和商品

    const query = req.query
    const { status, message, data } = await StatisticsTasks.queryOrdersByKey(
      query
    )
...
route.get('/:order_id', async (req, res, next) => {
  ...
  const result = await StatisticsTasks.queryOrderCommodityById(
    order_id,
    orderDetails.check_date
  )
