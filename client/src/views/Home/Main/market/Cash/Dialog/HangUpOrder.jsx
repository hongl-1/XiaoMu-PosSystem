...
function OrderList({ list, selectType, select, clickSelect }) {
  const data = useMemo(
    () =>
      list.map(({ id, vip, commodityList, time }) => {
        let _count = 0,
          _money = 0
...
export function HandUpOrder({ status, hideFn, list }) {
  ...
  function handleHotKey(e) {
    ...
    if (key === 'ArrowUp') {
      ...
      setSelectData({
        ...
        selectType: 'up',
        currentData: list.find(({ id }) => id === list[index - 1].id)
          .commodityList
      })
