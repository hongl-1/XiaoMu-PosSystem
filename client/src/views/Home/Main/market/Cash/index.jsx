  ...
  const Right = useMemo(
    () => (
      <CashRight
        show={showCashHotKey}
        hotkey={hotkey}
        count={hangupOrder.list.length}
      />
    ),
    [showCashHotKey, hotkey, hangupOrder]
  )
  ...
  const { count, money, origin_price } = useMemo(() => {
    ...
    commodityList.map(
      ({
        money: item_money,
        count: item_count,
        origin_price: item_origin_price
      }) => {
        count = mathc.add(count, item_count)
        money = mathc.add(money, item_money)
        origin_price = mathc.add(
          origin_price,
          mathc.multiply(item_origin_price, item_count)
        )
      }
    )
    return {
