...
export class StatisticsTasks {
  static async queryOrdersByTime(start_time, end_time) {
    ...
    const order_list = await OrdersTasks.getOrdersByTimerange(
      start_time,
      end_time
    )
  ...
  static async queryOrderCommodityById(order_id, timestamp) {
    ...
    const result = Promise.all(
      commodity_list.map(async ({ order_id, commodity_id, ...fields }) => {
        const { barcode, name, unit, size, in_price, pinyin } =
  ...
  static async queryOrdersByKey(values) {
    ...
    async function queryByName(orders) {
      ...
      for (let { id, order_id, check_date } of orders) {
        const commodity_list = await OrdersTasks.getOrderCommodityById(
          order_id,
          check_date,
          true
        )
        for (let { name, pinyin, id: commodity_id } of commodity_list) {
          ;(pinyin.includes(query) || name.includes(query)) &&
            data.push({
              id,
              order_id,
              commodity_id
  ...
  static async getPropotionByCategory(order_list, category) {
    ...
    for (let id of commodity_id_list) {
      const { category_id } = await CommodityTask.getCommodityDetails(
        id,
        'id',
        true
      )
  ...
  static async getPropotionByPosUser(order_list) {
    ...
    for (let { user_id, sale_price } of order_list) {
      if (user_check_count[user_id]) {
        user_check_count[user_id] = mathc.add(
          user_check_count[user_id],
          sale_price
        )
      } else {
  ...
  static async getPropotionBySupplier(order_list) {
    ...
    for (let id of commodity_id_list) {
      const { supplier_id } = await CommodityTask.getCommodityDetails(
        Number(id),
        'id',
        true
      )
  ...
  static async getSalesTrends(query) {
    ...
    const [_start_time, _end_time, timeKeyList] = createTimerangeKey(
      start_time,
      end_time,
      maxTime,
      type
    ...
    for (let { check_date, ...args } of order_list) {
      ...
      for (let key of keys) {
        ...
        vl[key] =
          vl[key] === undefined ? args[key] : mathc.add(vl[key], args[key])
      }
    }
