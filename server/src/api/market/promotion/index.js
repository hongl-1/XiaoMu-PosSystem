...
route.post(
  '/create',
  validBody(createPromotionSchema, '请输入正确的促销活动详情!'),
  async (req, res, next) => {
    // 创建促销活动
    ...
    const { lastID } = await PromotionTask.createPromotion(
      name,
      start_date,
      end_date,
      description
...
route.put(
  '/update',
  validBody(updatePromotionSchema, '请输入正确的促销活动信息!'),
  async (req, res, next) => {
    ...
    await PromotionTask.updatePromotionCommodity(
      queryNameResult.id,
      start_date,
      end_date
    )
...
route.post(
  ...
  async (req, res, next) => {
    ...
    const queryPromotionResult = await PromotionTask.getPromotion(
      promotion_name
    )
    if (!queryPromotionResult) {
    ...
    const { status, data } = await PromotionTask.validCommodityList(
      id,
      commodity_list
    )
    if (!status) {
