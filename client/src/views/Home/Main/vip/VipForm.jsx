...
function _VipForm({
  ...
  const { title, okText, submitFn, ...args } = initValue.find(
    (i) => i.type === type
  )
  ...
  const VipDisable = <Switch checked={is_disable} onChange={handleDisVip} />

  const VipTypeSelect = (
    <VipTypeSelectTree value={vipType} handleChange={handleVipType} />
  )
  const VipSexSelect = (
    <Radio.Group onChange={handleSexChange} value={vipSex}>
  ...
  const config =
    type === 'add' ? baseConfig : baseConfig.filter(({ edit = true }) => edit)
  ...
  async function handleUpdateVip(e) {
    ...
    try {
      const { data } = await VipManage.updateVipMember(
        ajax,
        selectItem.code,
        needUpdateValues
      )
  ...
  return (
    <Drawer
      ...
      title={title}
    >
      <Form {...formLayout} onSubmit={submitFn}>
        {config.map(({ label, component, id, initValue, rules = [] }) => (
        ...
        <Button block htmlType="submit" type="primary" disabled={!cantSubmit}>
          {okText}
        </Button>
      </Form>
    </Drawer>
