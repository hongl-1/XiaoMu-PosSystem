...
function _ChangeModal({
  ...
  const { title, okText, initialValue, onOk } = config.find(
    (i) => i.type === type
  )
          ...
          ({
            ...
            <Form.Item label={label} key={label}>
              {getFieldDecorator(id, {
                ...
                ComponentFn ? (
                  ComponentFn(disabled[type])
                ) : (
                  <Input type={_type} disabled={disabled[type]} />
                )
              )}
            </Form.Item>
          )
...
export function UserManageFn({
  ...
  return (
    <div className={styled['user-manage-fn']}>
      {config.map(
        ({ title, btnType = 'primary', icon, disabled = false, fn }) => (
          <Button
            type={btnType}
            key={title}
            icon={icon}
