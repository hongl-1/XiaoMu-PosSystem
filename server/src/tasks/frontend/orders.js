...
class OrdersTask {
  ...
  static async validOrderPrice(price, list) {
    ...
    const result = mathc.addList(
      list.map(({ sale_price, count }) => mathc.multiply(sale_price, count))
    )
  ...
  static async createOrderID() {
    ...
    const serial_number = await this.createSerialNumber(
      year,
      month,
      day,
      timestamp
    )
    const order_id = `${String(year).slice(2)}${format(month)}${format(
      day
    )}${String(timestamp).slice(5, 10)}${serial_number}`
  ...
  static async handleOrder({
    ...
    for (let item of commodity_list) {
      ...
      if (!isAdmin) {
        ...
        const [{ sale_price: _sale_price }] =
          await FrontCommodityTask.parseCommodityList([item])
        // 获得商品促销价格
  ...
  static async getOrderCommodityById(
    order_id,
    check_date,
    need_pinyin = false
  ) {
    ...
    return await Promise.all(
      (
        await this.getOrderDetails(order_id)
      ).map(async ({ id, order_id, commodity_id, ...args }) => {
        const { name, pinyin } =
          await CommodityTask.getCommodityDetailsByTimestamp(
            check_date,
            commodity_id,
            'id'
          )
  ...
  static async getOrderAllDetails(order_id) {
    ...
    const commodity_data = await this.getOrderCommodityById(
      order_id,
      result.check_date
    )
  ...
  static async saveOrderDetails(order_id, commodity_list_details) {
    ...
    return await Promise.all(
      commodity_list_details.map(async (args) => {
  ...
  static async getOrdersByTimerange(start_time, end_time, not_undo = false) {
    ...
    const whereKey = ['check_date >= ?', 'check_date <= ?']
  ...
  static async getOrdersByTimerangeAndUser(
    start_time,
    end_time,
    user_id,
    not_undo = false
    ...
    const whereKey = ['check_date >= ?', 'check_date <= ?', 'user_id = ?']
