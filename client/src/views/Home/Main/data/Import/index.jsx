...
export function DataImport() {
  ...
  const { currentTypeConfig, mapFields } = useMemo(() => {
    const currentTypeConfig = typeConfig.find(
      (i) => i.type === option['import_type'].value
    )

    const mapFields = currentTypeConfig.fields.filter(({ map = false }) => map)
  ...
  function formatData() {
    ...
    for (let { field, convertFn } of fields) {
      option[field] &&
        option[field].value &&
        (base_fields[field] = option[field].value) &&
        convertFn &&
        (base_fields_convert[field] = convertFn)
    }
    ...
    config.data = data.map((item) => {
      ...
      keys.forEach((key) => {
        ...
        value !== undefined &&
          value !== null &&
          (vl[key] = convertFn ? convertFn(value) : value)
      })
      return vl
    })
  ...
  const baseBtnList = [
    ...
    },
    {
      ...
      show:
        (currentStep === 3 &&
          (uploadStatus === 'wait' || uploadStatus === 'error')) ||
  ...
  return (
    <div className={styled['data-import-wrapper']}>
      <Steps current={currentStep}>
        {stepConfig.map(({ title }) => (
          <Step key={title} title={title} />
        ))}
      </Steps>
      <div className={styled['steps-body']}>{content}</div>
      <div className={styled['steps-btn-wrap']}>
        {btnList.map(({ title, disabled, type = 'primary', fn }) => (
          <Button key={title} disabled={disabled} type={type} onClick={fn}>
            {title}
          </Button>
        ))}
      </div>
