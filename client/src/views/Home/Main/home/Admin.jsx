...
function getLineChartsOption(data) {
  const dataSource =
    (data &&
      data.map(
        ({ time, sale_price = 0, in_price = 0, profit = 0, count = 0 }) => [
          time,
          sale_price,
          in_price,
          profit,
...
export function Admin({ status, data }) {
  const {
    category_data,
    weekend_data,
    sale_price,
  ...
  const Option = useMemo(
    () => getLineChartsOption(weekend_data),
    [weekend_data]
  )
  ...
  const base_category_data = (category_data &&
  ...
  return (
    <>
      <CountCard config={cardConfig} cssHook={handleCss} />
