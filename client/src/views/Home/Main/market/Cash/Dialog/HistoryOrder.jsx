...
export function HistoryOrder({ status, hideFn, list }) {
  ...
  function handleClickSelect(id) {
    setSelectData((s) => ({
      ...
      select: id,
      selectType: 'click',
      commodity_list: list.find(({ order_id }) => order_id === id)
        .commodity_list
    }))
  }

  const selectFlag =
    list.length > 0 && list.find(({ order_id }) => order_id === select)
  // 当前有选中订单的flag
    ...
    try {
      const list = await Promise.all(
        selectFlag.commodity_list.map(
          async ({ barcode }) =>
            (
              await CommodityTasks.query(ajax, barcode)
            ).data
        )
      )
      // 从后台获取当前商品信息
      ...
      list.map((arr) => {
        if (arr.length !== 1) {
          ...
          const { count } = selectFlag.commodity_list.find(
            ({ barcode }) => barcode === tmp.barcode
          )
          // 之前已完成订单里的商品信息
  ...
  const ModalFooter = useMemo(
    () => <BaseModalFooter hotkey={hotkey.filter(({ show = true }) => show)} />,
    [hotkey]
  )
  ...
  return (
    <div className={styled['modal-footer']}>
      {hotkey.map(({ status, title, key, fn, type = 'primary' }) => (
        <Button disabled={!status} key={title} type={type} onClick={fn}>
          {`${title} <${key}>`}
        </Button>
      ))}
    </div>
