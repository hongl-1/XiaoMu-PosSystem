...
class UsersTask {
  ...
  static async mapValueToText(list) {
    return await Promise.all(
      list.map(async ({ group_id, id, username, disabled }) => {
  ...
  static async createUser(username, hash, group_id) {
  ...
  static async getUserAuthority(username, needGroup = false) {
    ...
    const authorityList = await Promise.all(
      (
        await AppDAO.all(
          `
        ...
        AuthorityTask.getAuthorityDetails(authority_id)
      )
    )
    const result = {
      authorityList
