...
route.put(
  '/updatepwd',
  validBody(updateUserPwdSchema, '请输入正确的用户名和密码!'),
  async (req, res, next) => {
    // 修改用户密码
...
route.put(
  '/updateusergroup',
  validBody(updateUserGroupSchema, '请输入正确的信息!'),
  async (req, res, next) => {
    // 修改用户所属的用户组
...
route.post(
  '/updateusername',
  validBody(updateUserNameSchema, '请输入正确的用户名!'),
  async (req, res, next) => {
    // 修改用户名
    ...
    if (!queryOldUserResult) {
      return throwError(next, '此账户不存在，无法修改!')
    }
    // 如果被修改用户名的用户不存在返回400
...
route.post(
  '/createuser',
  validBody(newUserSchema, '请输入正确的信息!'),
  async (req, res, next) => {
    // 创建新用户
...
route.put(
  '/updateuserstatus',
  validBody(updateUserStatusSchema, '请输入正确的信息组合!'),
  async (req, res, next) => {
    // 设置用户是否被禁用
